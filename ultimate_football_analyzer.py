...
class UltimateFootballAnalyzer:
    ...
    def visualize_frame(self, frame, detections, team_labels, play_type, ball_position, frame_id):
        ...
        # 공 소유권 텍스트 처리
        ball_possession_text = "Unknown"
        if ball_position is not None and self.ball_detector.possession_team is not None:
            ball_possession_text = "Team 1"
        ...
        info = [
            ...
            f"Ball Possession: {ball_possession_text}"
        ]
...
def main():
    ...
    if mode == "2":
        ...
        analyzer = UltimateFootballAnalyzer('runs/football/train_xxx/weights/best.pt')
    else:
